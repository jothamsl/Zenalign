...
class DatasetProfiler:
    ...
    def __init__(self, df: pd.DataFrame, problem_type: str):
        ...
        logger.info(
            f"Initialized profiler for {problem_type} problem: {self.n_rows} rows x {self.n_cols} cols"
        )
            ...
            if method == "iqr":
                ...
                if len(outlier_values) > 0:
                    outliers[col] = {
                        ...
                        "percentage": round(
                            (len(outlier_values) / self.n_rows) * 100, 2
                        ),
    ...
    def calculate_quality_scores(self) -> Dict[str, float]:
        ...
        # Simple heuristic: percentage of columns with consistent data types
        consistent_cols = 0
        for col in self.df.columns:
            if self.df[col].dtype in ["int64", "float64", "bool"]:
                # Numeric columns are considered consistent if they don't have mixed types
                consistent_cols += 1
            elif self.df[col].dtype == "object":
                # Object columns are consistent if they don't have too much variety relative to size
                unique_ratio = self.df[col].nunique() / len(self.df[col].dropna())
                if (
                    unique_ratio < 0.8
                ):  # Less than 80% unique values suggests consistency
                    consistent_cols += 1
        scores["consistency"] = round(consistent_cols / max(self.n_cols, 1), 3)
    ...
    def get_issue_summary(self) -> List[Dict[str, Any]]:
        ...
        outlier_severity_boost = (
            1 if self.problem_type in ["regression", "time_series"] else 0
        )
        ...
        imbalance_critical = self.problem_type in [
            "classification",
            "anomaly_detection",
        ]
        ...
        for col, info in imbalance.items():
            severity = (
                "critical"
                if imbalance_critical and info["minority_percentage"] < 5
                else "high"
                if info["minority_percentage"] < 10
                else "medium"
                if info["minority_percentage"] < 20
                else "low"
            )
